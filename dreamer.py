...
def deepdreamHalf(net, base_img, iter_n=10, octave_n=4, step_size=1.5, octave_scale=1.4, end='inception_4c/output', clip=True, verbose = 1, maxWidth=1500, objective=objective_L2, targetId=0, **step_params):
    ...
    for octave, octave_base in enumerate(octaves[::-1]): # itterating backward
        ...
        if w > maxWidth:
            ...
            for i1 in range(0,2):
                for j1 in range(0,2):
                    spl_detail = detail[:,i1*int(h/2):(i1+1)*int(h/2), j1*int(w/2):(j1+1)*int(w/2)]
                    spl_octave_base = octave_base[:,i1*int(h/2):(i1+1)*int(h/2), j1*int(w/2):(j1+1)*int(w/2)]
                    ...
                    detail[:,i1*int(h/2):(i1+1)*int(h/2), j1*int(w/2):(j1+1)*int(w/2)] = spl_detail
                    octave_base[:,i1*int(h/2):(i1+1)*int(h/2), j1*int(w/2):(j1+1)*int(w/2)] = spl_octave_base
                    temp_data[:,i1*int(h/2):(i1+1)*int(h/2), j1*int(w/2):(j1+1)*int(w/2)] = src.data[0]
            #combine toghether
            src.reshape(1,3,h,w)
    ...
    if flow is 1:
        ...
        for v in range(len(vids)):
            if var_counter < len(vids):
                ...
                now = time.time()
                endparam = layers[var_counter % len(layers)]
                print("var_counter "+ str(var_counter))
